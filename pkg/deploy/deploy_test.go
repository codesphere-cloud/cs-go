	...
	Describe("Deploy", func() {
		...
		Context("create mode (no existing workspace)", func() {
			It("creates a new workspace and returns result", func() {
				devDomain := "300-3000.2.codesphere.com"
				// FindWorkspace returns nothing
				mockClient.EXPECT().ListWorkspaces(teamId).Return([]api.Workspace{}, nil)
				// CreateWorkspace
				mockClient.EXPECT().DeployWorkspace(mock.Anything).Return(&api.Workspace{Id: 300, Name: wsName, DevDomain: &devDomain}, nil)

				result, err := deployer.Deploy(cfg, false)
				Expect(err).ToNot(HaveOccurred())
				Expect(result).ToNot(BeNil())
				Expect(result.WorkspaceId).To(Equal(300))
				Expect(result.WorkspaceURL).To(Equal("https://300-3000.2.codesphere.com/"))
			})
		})

		Context("update mode (existing workspace)", func() {
			It("updates existing workspace and returns result", func() {
				devDomain := "200-3000.2.codesphere.com"
				existing := &api.Workspace{Id: 200, Name: wsName, DevDomain: &devDomain}
				...
				Expect(result.WorkspaceURL).To(Equal("https://200-3000.2.codesphere.com/"))
			})
		})
	})
})
