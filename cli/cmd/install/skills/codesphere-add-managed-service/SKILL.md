...
- **Shared family conventions apply — see `references/skill-family-conventions.md`** (inside `codesphere`'s directory — `Glob` for `**/codesphere/references/*.md` if the install path isn't already known). This skill ships its own `references/interview-<name>.md` files (resolved locally, inside this skill's own directory), but every other `references/*.md` path used below (`providers.md`, `provider-<name>.md`, `ci-pipeline.md`, `secret-management.md`, `skill-family-conventions.md` itself) lives inside the sibling `codesphere` skill's directory instead — never requires `codesphere` itself to be loaded as an active skill. If `codesphere` can't be located at all, stop and tell the user it needs to be installed alongside this skill — don't guess at provider field content from memory.
