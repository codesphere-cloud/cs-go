...
func AddStartPipelineCmd(start *cobra.Command, opts shared.RootOptions) {
	pipeline := StartPipelineCmd{
		...
		Opts: StartPipelineOpts{RootOptions: opts},
		Time: &api.RealTime{},
	}
	...
	shared.AddCmd(start, pipeline.cmd)
...
func (c *StartPipelineCmd) allFinished(status []api.PipelineStatus) bool {
	io.Verboseln(c.Opts.GetVerbose(), "====")
	for _, s := range status {
		io.Verbosef(c.Opts.GetVerbose(), "Server: %s, State: %s, Replica: %s\n", s.Server, s.State, s.Replica)
	}
	for _, s := range status {
